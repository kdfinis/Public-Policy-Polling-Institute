...
// Get service account from environment variable (GitHub Secrets)
...
async function seedUsers() {
  ...
  const existingUsers = await db.collection('users').where('is_demo', '==', true).limit(1).get();
  if (!existingUsers.empty && process.env.FORCE_SEED !== 'true') {
    console.log('⚠️  Demo users already exist. Skipping user seed. Set FORCE_SEED=true to overwrite.\n');
    // Still return existing user IDs for vote seeding
    const allUsers = await db.collection('users').where('is_demo', '==', true).get();
    return allUsers.docs.map(d => d.id);
  }
